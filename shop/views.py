...
def home(request,c_slug=None):
    ...
    if c_slug != None:
        ...
        prodt=products.objects.filter(category=c_page,availability=True)
    else:
        prodt=products.objects.all().filter(availability=True)
...
def searching(request):
    ...
    if 'q' in request.GET:
        ...
        pro=products.objects.all().filter(Q(name__contains=query)|Q(desc__contains=query))
